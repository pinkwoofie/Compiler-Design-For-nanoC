%{
    ...
    */
    ...
    extern vector<string> funcParam;
...
direct_declarator: 
        ...
        | direct_declarator '(' parameter_type_list_opt ')'
        {
            ...
            cout<<"Direct-Declarator : "<<$1->name<<endl;
            for(int i = 0; i < (int)paramList.size(); i++)
            {
                param* curParam = paramList[i];
                if(curParam->type.type == ARRAY) {          // If the parameter is an array
                    if(curParam->type.nextType == CHAR) funcParam.push_back("char[]");
                    else if(curParam->type.nextType == INT) funcParam.push_back("int[]");
                }
                else if(curParam->type.type == POINTER) {   // If the parameter is a pointer
                    if(curParam->type.nextType == INT) funcParam.push_back("int*");
                    else if(curParam->type.nextType == CHAR) funcParam.push_back("char*");
                }
                else {
                    if(curParam->type.type == VOID)
                        funcParam.push_back("void");
                    else if(curParam->type.type == CHAR)
                        funcParam.push_back("char");
                    else if(curParam->type.type == INT)
                        funcParam.push_back("int");
                }
            }
            for(int i = 0; i < (int)paramList.size(); i++) {
                param* curParam = paramList[i];
...
function_definition: 
        ...
        {
            DataType t = $1;
            if($1 == CHAR) returnType = "char";
            else if($1 == INT) returnType = "int";
