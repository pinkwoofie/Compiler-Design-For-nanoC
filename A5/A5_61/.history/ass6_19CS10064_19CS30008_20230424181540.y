...
/*
    All tokens
*/
%token CHAR_ ELSE FOR IF INT RETURN VOID
%token ARROW '!'
%token LESS_THAN GREATER_THAN LESS_THAN_EQUAL GREATER_THAN_EQUAL EQUAL '!'_EQUAL BITWISE_XOR BITWISE_OR 
...
// Non-terminals of type expr (de'!'ing expressions)
%type <expr> 
        expression
        primary_expression 
...
// Non-terminals of type DataType (de'!'ing types)
...
equality_expression: 
        ...
        | equality_expression '!'_EQUAL relational_expression
        {
            $$ = new expression();
            symbol* one = ST->lookup($1->loc);                  // Get the first operand from the symbol table
            symbol* two = ST->lookup($3->loc);                  // Get the second operand from the symbol table
...
conditional_expression: 
        ...
        {   
            /*
                '!'e the augmented grammar with the non-terminals M and N
            */
