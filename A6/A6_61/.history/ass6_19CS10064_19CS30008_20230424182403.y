%{
    ...
    */
    ...
    extern CHAR_* yytext;                    // From lexer, gives the text being currently scanned
...
%union {
    ...
    CHAR_ charval;                   // For a CHAR_ value
...
primary_expression: 
        ...
        {
            $$ = new expression();                  // Create new expression
            $$->loc = ST->gentemp(CHAR_);            // Generate a new temporary variable
            emit($$->loc, $1, ASSIGN);
            symbolValue* val = new symbolValue();
            val->setInitVal($1);                    // Set the initial value
            ST->lookup($$->loc)->initVal = val;     // Store in symbol table
