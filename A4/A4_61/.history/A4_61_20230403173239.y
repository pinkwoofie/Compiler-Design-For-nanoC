...
primary_expression:
					...
					|'-'INTEGER_CONSTANT {printf("Line No : %d) primary_expression --> negative integer constant\n",yylineno);}
...
statement:
		 ...
		 |matched_statemt {printf("Line No : %d) statement --> matched_statement\n",yylineno);}
		 |open_statement {printf("Line No : %d) statement --> open_statement\n",yylineno);}
		 ;
...
struct symbol_table_entry* find_symbol(char* name) {
    int index = hash(name);
    struct symbol_table_entry* entry = symbol_table[index];
    while (entry != NULL) {
